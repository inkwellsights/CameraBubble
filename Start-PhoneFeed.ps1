# Start-PhoneFeed.ps1 - just the hidden phone camera feed, for OBS to capture.
# Use this when you want to STREAM / RECORD in OBS with the phone as a source
# (instead of PhoneWebcam.bat, which would fight OBS over the virtual camera).
#   PhoneFeed.bat                 (asks rear/front)
#   PhoneFeed.bat -Camera front   (skip the prompt)
param([string]$Camera = "", [string]$Connect = "")
$ErrorActionPreference = "Stop"
$Res = "1920x1080"; $Fps = "30"

# --- stop the gesture engine + bubble + virtual-cam feeder (clean dedicated feed) ---
...
Write-Host ""
Write-Host "Phone feed is running (hidden, window title 'PhoneCam')." -ForegroundColor Green
Write-Host "In OBS:  Sources -> + -> Window Capture -> '[scrcpy.exe]: PhoneCam'" -ForegroundColor Green
Write-Host "         Then set Capture Method = 'Windows 10 (1903 and up)'  (NOT BitBlt, or it's black)." -ForegroundColor Yellow
Write-Host "Now stream / record normally. You can close this window; the feed keeps running." -ForegroundColor Green
Write-Host "Stop everything later with Stop-PhoneSuite.bat." -ForegroundColor Gray
